...
const stickerCommand = async (m, fox) => {
  ...
  if (config.AUTO_STICKER && !m.key.fromMe) {
    if (m.type === 'imageMessage') {
      ...
      await fox.sendImageAsSticker(m.from, mediac, m, { packname, author });
      console.log(`Auto sticker detected`);
      return;
    } else if (m.type === 'videoMessage' && m.msg.seconds <= 11) {
      let mediac = await m.download();
      await fox.sendVideoAsSticker(m.from, mediac, m, { packname, author });
      return;
    }
  }
  ...
  if (validCommands.includes(cmd)) {
    ...
    if (quoted.mtype === 'imageMessage') {
      ...
      await fox.sendImageAsSticker(m.from, stickerBuffer, m, { packname, author });
    } else if (quoted.mtype === 'videoMessage') {
      await fox.sendVideoAsSticker(m.from, filePath, m, { packname, author });
    }
  }
};
