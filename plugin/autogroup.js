...
const groupSetting = async (m, fox) => {
  try {
    ...
    if (!time) {
      if (groupSetting === 'close') {
        await fox.groupSettingUpdate(m.from, 'announcement');
        return m.reply("Group successfully closed.");
      } else if (groupSetting === 'open') {
        await fox.groupSettingUpdate(m.from, 'not_announcement');
    ...
    scheduledTasks[m.from] = cron.schedule(cronTime, async () => {
      try {
        ...
        if (groupSetting === 'close') {
          await fox.groupSettingUpdate(m.from, 'announcement');
          await fox.sendMessage(m.from, { text: "Group successfully closed." });
        } else if (groupSetting === 'open') {
          await fox.groupSettingUpdate(m.from, 'not_announcement');
          await fox.sendMessage(m.from, { text: "Group successfully opened." });
        }
      } catch (err) {
        console.error('Error during scheduled task execution:', err);
        await fox.sendMessage(m.from, { text: 'An error occurred while updating the group setting.' });
      }
    }, {
