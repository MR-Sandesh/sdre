...
import fs from 'fs';
import { getRandom } from '../auth_lib/script.cjs';  // Assuming you have a utility function for generating random file names

const audioEffects = async (m, fox) => {
  try {
    ...
    exec(`ffmpeg -i ${mediaPath} ${set} ${outputPath}`, (err, stderr, stdout) => {
      ...
      fox.sendMessage(m.from, { audio: buff, mimetype: 'audio/mpeg' }, { quoted: m });
