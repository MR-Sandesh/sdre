...
const quotedChat = async (m, fox) => {
  try {
    ...
    try {
      profilePicture = await fox.profilePictureUrl(m.quoted ? m.quoted.sender : m.sender, 'image');
    } catch {
    ...
    const quoteObject = {
      ...
      messages: [{
        ...
        from: {
          id: 1,
          name: m.quoted ? (await fox.getContact(m.quoted.sender)).notify || m.quoted.sender.split('@')[0] : m.pushName,
          photo: {
            url: profilePicture
          }
        },
    ...
    try {
      ...
      await fox.sendImageAsSticker(m.from, buffer, m, {
        packname: "",
        author: "> Ethix-MD"
      });
    } catch (error) {
