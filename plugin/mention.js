...
const mistral = async (m, Fox) => {
    ...
    const botNumber = await Fox.decodeJid(Fox.user.id).split('@')[0];
    ...
    if (isBotOrOwnerMentioned) {
        ...
        if (text === `@${botNumber}` || text === `@${ownerNumber}`) {
            await Fox.sendMessage(m.from, { audio: { url: 'https://telegra.ph/file/2d742503794850627fbdd.mp4' } }, { quoted: m });
            return;
        }
        ...
        if (strippedText) {
            try {
                ...
                const response = await fetch('https://Foxcoder.tech/api/ai', {
                    method: 'POST',
                    headers: {
