const invite = async (m, fox) => {
  try {
    ...
    const group = m.from;
    const groupMetadata = await fox.groupMetadata(group);
    const link = 'https://chat.whatsapp.com/' + await fox.groupInviteCode(group);
    ...
    await fox.sendMessage(`${text}@s.whatsapp.net`, { text: inviteMessage, mentions: [m.sender] });
